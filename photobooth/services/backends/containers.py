...
def init_res_obj_backend(_obj_: AbstractBackend):
...
class BackendsContainer(containers.DeclarativeContainer):
    ...
    digicamcontrol_backend = providers.Resource(init_res_obj_backend, DigicamcontrolBackend)
    gphoto2_backend = providers.Object(None)
    picamera2_backend = providers.Object(None)
    virtualcamera_backend = providers.Resource(init_res_obj_backend, VirtualCameraBackend)
    webcamcv2_backend = providers.Resource(init_res_obj_backend, WebcamCv2Backend)
    ...
    try:
        ...
        picamera2_backend = providers.Resource(init_res_obj_backend, Picamera2Backend)
    ...
    try:
        ...
        gphoto2_backend = providers.Resource(init_res_obj_backend, Gphoto2Backend)
    ...
    try:
        ...
        webcamv4l_backend = providers.Resource(init_res_obj_backend, WebcamV4lBackend)
    ...
    primary_backend = providers.Selector(
        providers.Callable(lambda cfg_enum: cfg_enum.backends.MAIN_BACKEND.lower(), cfg_enum=appconfig),
        **backends_set,
    )

    secondary_backend = providers.Selector(
        providers.Callable(lambda cfg_enum: cfg_enum.backends.LIVE_BACKEND.lower(), cfg_enum=appconfig),
        **backends_set,
    )
