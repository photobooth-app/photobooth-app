...
def add_hotkey(hotkey, callback, args=(), suppress=False, timeout=1, trigger_on_release=False):
    ...
    if len(steps) == 1:
        ...
        # for.
        def handler(e):
            return event_type == KEY_DOWN and e.event_type == KEY_UP and e.scan_code in _logically_pressed_keys or event_type == e.event_type and callback()
...
def get_hotkey_name(names=None):
    ...
    def sorting_key(k):
        return modifiers.index(k) if k in modifiers else 5, str(k)
...
def read_hotkey(suppress=True):
    ...
    queue = _queue.Queue()
    def fn(e):
        return queue.put(e) or e.event_type == KEY_DOWN
