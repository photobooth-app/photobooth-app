...
TEST_KEY_DEFAULT_VALUE = configsettings.ConfigSettings().get_schema()["properties"][
    TEST_GROUP
]["default"][TEST_KEY]
...
def tmp_envfile(tmp_movealloutoftheway, request):
    with open(request.param, "w", encoding="utf-8") as env_file:
        env_file.write(f"{TEST_GROUP}__{TEST_KEY}='{TEST_KEY_TEST_VALUE}'")
...
def test_defaultsetting(tmp_movealloutoftheway):
    assert (
        configsettings.ConfigSettings().common.CAPTURE_CAM_RESOLUTION_HEIGHT
        == TEST_KEY_DEFAULT_VALUE
    )
...
def _separate_process_settings_passed_fun(
    checkValue: Value, settings: configsettings.ConfigSettings
):
    checkValue.value = settings.common.CAPTURE_CAM_RESOLUTION_HEIGHT
...
def test_settings_available_in_separate_spawned_process():
    ...
    _p1 = Process(
        target=_separate_process_no_settings_passed_fun,
        args=(check_value1,),
        daemon=True,
    )
    _p1.start()
    ...
    _p2 = Process(
        target=_separate_process_settings_passed_fun,
        args=(check_value2, settings),
        daemon=True,
    )
    _p2.start()
    # wait for join, that the process has finished
