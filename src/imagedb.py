...
def _db_imageitem(filepath: str, user_caption: str = ""):
    ...
    if not (
        Path(item['image']).is_file() and
        Path(item['preview']).is_file() and
        Path(item['thumbnail']).is_file()
    ):
...
class ImageDb():
    ...
    def capture_hq_image(self, requested_filepath=None, copy_for_compatibility=False):
        ...
        try:
            ...
            (item, _) = self.create_imageset_from_image(
            ...
            self._evtbus.emit("publishSSE", sse_event="imagedb/newarrival",
